...
if(isset($_GET['enviarMensagem'])){
    if(enviarMensagem($_POST['user_id'],$_POST['msg'])){
...
foreach($chats as $chat){
    ...
    <div class="d-flex justify-content-between border-bottom chatlist_item" data-bs-toggle="modal" data-bs-target="#chatbox" onclick="popchat('.$chat['user_id'].')" >
                        <div class="d-flex align-items-center p-2">
                            <div><img src="assets/recursos/profile/'.$ch_user['profile_pic'].'" alt="" height="40" width="40" class="rounded-circle border">
                            </div>
                            <div>&nbsp;&nbsp;</div>
...
$messages = getMensagens($_POST['chatter_id']);
$chatmsg="";
...
attStatusVisualizado($_POST['chatter_id']);
...
$json['contadorDeMensagem']=contadorDeMensagem();
echo json_encode($json);
}

if(isset($_GET['unblock'])){
  $user_id = $_POST['user_id']; 
    if(desbloquearUsuario($user_id)){
        $response['status']=true;
    }else{
        $response['status']=false;
    }
...
if(isset($_GET['adicionarComentario'])){
    $post_id = $_POST['post_id'];
    $comment = $_POST['comment'];


    
        if(adicionarComentario($post_id,$comment)){
      $cuser = getUser($_SESSION['userdata']['id']);
      $time = date("Y-m-d H:i:s");
            $response['status']=true;
            $response['comment']='<div class="d-flex align-items-center p-2">
            <div><img src="assets/recursos/profile/'.$cuser['profile_pic'].'" alt="" height="40" class="rounded-circle border">
            </div>
            <div>&nbsp;&nbsp;&nbsp;</div>
...
$users="";
    if(count($data)>0){
       ...
       $users.=' <div class="d-flex justify-content-between">
                            <div class="d-flex align-items-center p-2">
                                <div><img src="assets/recursos/profile/'.$fuser['profile_pic'].'" alt="" height="40" class="rounded-circle border">
                                </div>
                                <div>&nbsp;&nbsp;</div>
                                <div class="d-flex flex-column justify-content-center">
