...
if(isset($_GET['block'])){
  ...
  if(isset($_GET['deletarPost'])){
    $post_id = $_GET['deletarPost'];
      if(deletarPost($post_id)){
...
$response=validarCadastro($_POST);
if($response['status']){
    if(createUser($_POST)){
...
if(isset($_GET['login'])){
    ...
    $response=validarFormularioLogin($_POST);
...
if(isset($_GET['forgotpassword'])){
    ...
    }elseif(!emailRegistrado($_POST['email'])){
...
if(isset($_GET['addpost'])){
   ...
   $responseMusic = validatePostMusic($_FILES['post_music']);

   if($response['status'] || $responseMusic['status']){
if(criarPost($_POST,$_FILES['post_img'], $_FILES['post_music'])){
